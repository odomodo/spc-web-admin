 ...
 * @LastEditTime: 2022-07-04 09:00:27
			...
			<section class="section_input">
				...
				<el-row class='spc-el-row'>
					...
					<el-col :span="20" style="text-align: left">
						<el-select clearable placeholder="请选择" style="width: 100%" v-model="taskSettingForm.serviceId">
							<el-option :label="item.deptName" :value="item.id" v-for="(item, index) in ServeNameDnList" :key="index">
								<span style="float: left">{{ item.serviceName }}</span>
								<span style="float: right; color: #8492a6; font-style: 14px">{{ item.serviceCode }}</span>
							</el-option>
						</el-select>
					</el-col>
				</el-row>
				<el-row class='spc-el-row'>
					...
					<el-col :span="20">
						...
						</el-input><el-button  :icon="Calendar" @click="cronVisible = true"></el-button>
...
const {dialogTitle,dialogVisible,taskSettingForm,ServeNameDnList,cronVisible} = toRefs(state)
const emit = defineEmits(['queryList'])
const cronCanel = () => {
	state.taskSettingForm.cronDetail = '';
...
const addSave = async (taskSettingForm: any) => {
	if (!taskSettingForm.taskCode || !taskSettingForm.taskName || !taskSettingForm.serviceId || !taskSettingForm.cronDetail) {
		ElMessage({
			message: '必填项不能为空!',
			type: 'error',
			duration: 3000,
	...
	const res:any = await addList(taskSettingForm);
	if (res.code == 0) {
		ElMessage({
			message: res.msg,
			type: 'success',
