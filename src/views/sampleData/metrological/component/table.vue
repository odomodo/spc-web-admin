...
<template>
	<div class="input-table">
		<div class="input-table-header">
			...
			<el-select v-model="filterType" size="large" placeholder="请选择筛选条件" style="width: 120px; margin-right: 10px; margin-left: 10px">
			...
			<div class="input-table-header-button" v-loading="loading" @click="initCharts(tableConfig.parentId)">
	...
	if (row.judgeStatus == 1 && type == 1) {
		...
		getOutAuditList(row.id).then((res: any) => {
			if (res.code == 0 && res.data.length > 0) {
				...
				//这期只有一个固定的审核人，所以这样写，后面添加多个审核人再优化
